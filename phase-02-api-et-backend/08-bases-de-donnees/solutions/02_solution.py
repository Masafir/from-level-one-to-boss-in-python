"""Module 08 — Solution exercice à trou #2"""
...
with Session(engine) as session:
    stmt = select(Game.genre, func.count(Game.id).label("count")).group_by(Game.genre)
    print("📊 Jeux par genre :")
    for row in session.execute(stmt): print(f"  {row.genre}: {row.count}")

    stmt = select(Game.genre, func.avg(Game.price).label("avg_price")).group_by(Game.genre).order_by(func.avg(Game.price).desc())
    print("\n💰 Prix moyen par genre :")
    for row in session.execute(stmt): print(f"  {row.genre}: {row.avg_price:.2f}€")

    stmt = (select(Player.name, Game.title, Score.value)
        .join(Score, Player.id == Score.player_id).join(Game, Game.id == Score.game_id)
        .order_by(Score.value.desc()).limit(5))
    print("\n🏆 Top 5 scores :")
    for row in session.execute(stmt): print(f"  {row.name} — {row.title}: {row.value}")

    stmt = (select(Player.name, func.count(Score.id).label("num_scores"), func.avg(Score.value).label("avg_score"))
        .join(Score, Player.id == Score.player_id).group_by(Player.id).having(func.count(Score.id) > 2))
    print("\n⭐ Joueurs actifs (>2 scores) :")
    for row in session.execute(stmt): print(f"  {row.name}: {row.num_scores} scores (moy: {row.avg_score:.0f})")

with Session(engine) as session:
    stmt = update(Game).where(Game.genre == "rpg").values(price=Game.price * 1.1)
    session.execute(stmt); session.commit()
    rpgs = session.scalars(select(Game).where(Game.genre == "rpg")).all()
    print("\n📈 RPGs après augmentation :")
    for g in rpgs: print(f"  {g.title}: {g.price:.2f}€")

    stmt = delete(Score).where(Score.value < 8000)
    result = session.execute(stmt); session.commit()
